            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="portlet light bordered">
                        ...
                        {{--                        @permission('add_apphomes')--}}
                        <div class="table-toolbar">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group">
                                        <a href="{{ url(route('dashboard.apphomes.create')) }}" class="btn sbold green">
                        ...
                        <div class="portlet-body">
                            <table class="table table-striped table-bordered table-hover" id="dataTable">
                                ...
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('apps::dashboard.app_homes.datatable.order')}}</th>
                                    <th>{{__('apps::dashboard.app_homes.datatable.title')}}</th>
                                    <th>{{__('apps::dashboard.app_homes.datatable.status')}}</th>
                                    <th>{{__('apps::dashboard.app_homes.datatable.type')}}</th>
                                    <th>{{__('apps::dashboard.app_homes.datatable.created_at')}}</th>
                                    <th>{{__('apps::dashboard.app_homes.datatable.options')}}</th>
                                </tr>
                                </thead>
                            </table>
                        </div>
                        <div class="row">
                            <div class="form-group">
                                <button type="submit" id="deleteChecked" class="btn red btn-sm"
                                        onclick="deleteAllChecked('{{ url(route('dashboard.apphomes.deletes')) }}')">
        ...
        function tableGenerate(data = '') {
            ...
            var dataTable =
                $('#dataTable').DataTable({
                    ...
                    ajax: {
                        url: "{{ url(route('dashboard.apphomes.datatable')) }}",
                        type: "GET",
                        data: {
                            req: data,
                        },
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            width: '13%',
                            title: '{{__('apps::dashboard.app_homes.datatable.options')}}',
                            className: 'dt-center',
                            orderable: false,
                            render: function (data, type, full, meta) {

                                // Edit
                                var editUrl = '{{ route("dashboard.apphomes.edit", ":id") }}';
                                editUrl = editUrl.replace(':id', data);

                                // Delete
                                var deleteUrl = '{{ route("dashboard.apphomes.destroy", ":id") }}';
                                deleteUrl = deleteUrl.replace(':id', data);

                                return `
      					@permission('edit_apphomes')
      						<a href="` + editUrl + `" class="btn btn-sm blue" title="Edit">
      			              <i class="fa fa-edit"></i>
      			            </a>
      					@endpermission

                        @permission('delete_apphomes')
                        @csrf
