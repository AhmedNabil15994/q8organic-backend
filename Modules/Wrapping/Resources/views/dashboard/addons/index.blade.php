...
@section('title', __('wrapping::dashboard.addons.routes.index'))
@section('content')
        ...
        <div class="page-content">
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="#">{{__('wrapping::dashboard.addons.routes.index')}}</a>
                    </li>
                </ul>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="portlet light bordered">
                        ...
                        @permission('add_wrapping_addons')
                        <div class="table-toolbar">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group">
                                        <a href="{{ url(route('dashboard.wrapping_addons.create')) }}"
                        ...
                        <div class="portlet-body">
                            <table class="table table-striped table-bordered table-hover" id="dataTable">
                                ...
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('wrapping::dashboard.addons.datatable.image')}}</th>
                                    <th>{{__('wrapping::dashboard.addons.datatable.status')}}</th>
                                    <th>{{__('wrapping::dashboard.addons.datatable.title')}}</th>
                                    <th>{{__('wrapping::dashboard.addons.datatable.created_at')}}</th>
                                    <th>{{__('wrapping::dashboard.addons.datatable.options')}}</th>
                                </tr>
                                </thead>
                            </table>
                        </div>
                        <div class="row">
                            <div class="form-group">
                                <button type="submit" id="deleteChecked" class="btn red btn-sm"
                                        onclick="deleteAllChecked('{{ url(route('dashboard.wrapping_addons.deletes')) }}')">
        ...
        function tableGenerate(data = '') {
            ...
            var dataTable =
                $('#dataTable').DataTable({
                    ...
                    ajax: {
                        url: "{{ url(route('dashboard.wrapping_addons.datatable')) }}",
                        type: "GET",
                        data: {
                            req: data,
                        },
...
responsivePriority: 1,
                            width: '210px',
                            title: '{{__('wrapping::dashboard.addons.datatable.options')}}',
                            className: 'dt-center',
                            orderable: false,
                            render: function (data, type, full, meta) {

                                // Edit
                                var editUrl = '{{ route("dashboard.wrapping_addons.edit", ":id") }}';
                                editUrl = editUrl.replace(':id', data);

                                // Delete
                                var deleteUrl = '{{ route("dashboard.wrapping_addons.destroy", ":id") }}';
                                deleteUrl = deleteUrl.replace(':id', data);

                                return `

                                    @permission('edit_wrapping_addons')
                                                <a href="` + editUrl + `" class="btn btn-sm blue" title="Edit">
                                                  <i class="fa fa-edit"></i>
                                                </a>
                                            @endpermission

                                    @permission('delete_wrapping_addons')
                                    @csrf
