...
@section('title', __('wrapping::dashboard.gifts.routes.index'))
@section('content')
        ...
        <div class="page-content">
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="#">{{__('wrapping::dashboard.gifts.routes.index')}}</a>
                    </li>
                </ul>
            </div>
                    ...
                    <div class="portlet light bordered">
                        ...
                        <div class="table-toolbar">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group">
                                        <a href="{{ url(route('dashboard.gifts.create')) }}" class="btn sbold green">
                        ...
                        <div class="portlet-body">
                            <table class="table table-striped table-bordered table-hover" id="dataTable">
                                ...
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('wrapping::dashboard.gifts.datatable.image')}}</th>
                                    <th>{{__('wrapping::dashboard.gifts.datatable.status')}}</th>
                                    <th>{{__('wrapping::dashboard.gifts.datatable.title')}}</th>
                                    <th>{{__('wrapping::dashboard.gifts.datatable.created_at')}}</th>
                                    <th>{{__('wrapping::dashboard.gifts.datatable.options')}}</th>
                                </tr>
                                </thead>
                            </table>
                        </div>
                        <div class="row">
                            <div class="form-group">
                                <button type="submit" id="deleteChecked" class="btn red btn-sm"
                                        onclick="deleteAllChecked('{{ url(route('dashboard.gifts.deletes')) }}')">
        ...
        function tableGenerate(data = '') {
            ...
            var dataTable =
                $('#dataTable').DataTable({
                    ...
                    ajax: {
                        url: "{{ url(route('dashboard.gifts.datatable')) }}",
                        type: "GET",
                        data: {
                            req: data,
                        },
...
responsivePriority: 1,
                            width: '210px',
                            title: '{{__('wrapping::dashboard.gifts.datatable.options')}}',
                            className: 'dt-center',
                            orderable: false,
                            render: function (data, type, full, meta) {

                                // Edit
                                var editUrl = '{{ route("dashboard.gifts.edit", ":id") }}';
                                editUrl = editUrl.replace(':id', data);

                                // Delete
                                var deleteUrl = '{{ route("dashboard.gifts.destroy", ":id") }}';
                                deleteUrl = deleteUrl.replace(':id', data);

                                return `

                                    @permission('edit_gifts')
                                                <a href="` + editUrl + `" class="btn btn-sm blue" title="Edit">
                                                  <i class="fa fa-edit"></i>
                                                </a>
                                            @endpermission

                                    @permission('delete_gifts')
                                    @csrf
