...
trait CrudCreator
{
    ...
    protected function getNewPath($file)
    {
        $path = array_key_exists($file, $this->paths) ?
            base_path(str_replace(['::module', '::folder','::model'], [$this->module, $this->folder , $this->model], $this->paths[$file])) : '';
    ...
    protected function getNameSpace($file)
    {
        $nameSpace = array_key_exists($file, $this->nameSpaces) ?
            str_replace(
                ['::module', '::folder'],
                [$this->module, str_replace('/' , '\\',$this->folder)],
    ...
    protected function getClassNames()
    {
        ...
        foreach ($this->class_names as $file => $class_name) {
            $classes += [$file => str_replace('::model', $this->model, $class_name)];
        }
    ...
    protected function buildRoute($controller)
    {
        ...
        return str_replace(['{{ route }}', '{{ controller }}'], [Str::plural(Str::lower($this->model)), $controller], $stub);
    }
}
