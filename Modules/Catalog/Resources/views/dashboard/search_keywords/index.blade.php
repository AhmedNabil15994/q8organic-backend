...
@section('title', __('catalog::dashboard.search_keywords.routes.index'))
@section('content')
        ...
        <div class="page-content">
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="#">{{__('catalog::dashboard.search_keywords.routes.index')}}</a>
                    </li>
                </ul>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="portlet light bordered">
                        ...
                        @permission('add_search_keywords')
                        <div class="table-toolbar">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group">
                                        <a href="{{ url(route('dashboard.search_keywords.create')) }}"
                        ...
                        <div class="portlet-body">
                            <table class="table table-striped table-bordered table-hover" id="dataTable">
                                ...
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('catalog::dashboard.search_keywords.datatable.title')}}</th>
                                    {{--<th>{{__('catalog::dashboard.search_keywords.datatable.status')}}</th>--}}
                                    <th>{{__('catalog::dashboard.search_keywords.datatable.created_at')}}</th>
                                    <th>{{__('catalog::dashboard.search_keywords.datatable.options')}}</th>
                                </tr>
                                </thead>
                            </table>
                        </div>
                        <div class="row">
                            <div class="form-group">
                                <button type="submit" id="deleteChecked" class="btn red btn-sm"
                                        onclick="deleteAllChecked('{{ url(route('dashboard.search_keywords.deletes')) }}')">
        ...
        function tableGenerate(data = '') {
            ...
            var dataTable =
                $('#dataTable').DataTable({
                    ...
                    ajax: {
                        url: "{{ url(route('dashboard.search_keywords.datatable')) }}",
                        type: "GET",
                        data: {
                            req: data,
                        },
...
responsivePriority: 1,
                            width: '210px',
                            title: '{{__('catalog::dashboard.search_keywords.datatable.options')}}',
                            className: 'dt-center',
                            orderable: false,
                            render: function (data, type, full, meta) {

                                // Edit
                                var editUrl = '{{ route("dashboard.search_keywords.edit", ":id") }}';
                                editUrl = editUrl.replace(':id', data);

                                // Delete
                                var deleteUrl = '{{ route("dashboard.search_keywords.destroy", ":id") }}';
                                deleteUrl = deleteUrl.replace(':id', data);

                                return `

                                    @permission('edit_search_keywords')
                                                <a href="` + editUrl + `" class="btn btn-sm blue" title="Edit">
                                                  <i class="fa fa-edit"></i>
                                                </a>
                                            @endpermission

                                    @permission('delete_search_keywords')
                                    @csrf
