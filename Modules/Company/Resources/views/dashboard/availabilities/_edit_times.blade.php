    ...
    <div class="col-md-12">
        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            @foreach(getDays() as $k => $day)
                    ...
                    <td>
                        <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                            <input type="checkbox"
                                   ...
                                   {{ $company->availabilities()->where('day_code', $k)->value('status') == 1 ? 'checked' : '' }}
                                   value="{{ $k }}"
                                   name="days_status[]">
                            <span></span>
                        </label>
                        ...
                        <div class="form-check form-check-inline">
                            <span class="is_full_day">
                                <input class="form-check-input check-time" type="radio" name="is_full_day[{{$k}}]"
                                       id="full_time-{{$k}}"
                                       value="1"
                                       {{ $company->availabilities()->where('day_code', $k)->first() == null || $company->availabilities()->where('day_code', $k)->first()->is_full_day == 1 ? 'checked' : '' }}
                            ...
                            <span class="is_full_day">
                                <input class="form-check-input check-time" type="radio" name="is_full_day[{{$k}}]"
                                       id="custom_time-{{$k}}"
                                       value="0"
                                       {{ isset($company->availabilities()->where('day_code', $k)->first()->is_full_day) && $company->availabilities()->where('day_code', $k)->first()->is_full_day == 0 ? 'checked' : '' }}
                ...
                @if(isset($company->availabilities()->where('day_code', $k)->first()->is_full_day) && $company->availabilities()->where('day_code', $k)->first()->is_full_day == 0)
                    ...
                    <tr id="collapse-{{$k}}" class="">
                        <td colspan="3" id="div-content-{{$k}}">
                            ...
                            @foreach($company->availabilities()->where('day_code', $k)->first()->custom_times as $key => $time)
